...
''', unsafe_allow_html=True)
    ...
    model_choice = st.radio(
        ...
        ('Complex Model with Management Fees and Profit Share Threshold', 
         'Simple Model with Conditional 40% Profit Share'
         )
    )
    ...
    **Model 1: Complex Model with Management Fees and Profit Share Threshold**  
    ...
    - **Profit Sharing:** 30% sharing of profits only after an 6% return on capital is achieved.

    **Model 2: Simple Model with Conditional 30% Profit Share**  
    - **Profit Sharing:** 40% sharing on profits exceeding a threshold level of capital.
    ''')
    ...
    if model_choice == 'Complex Model with Management Fees and Profit Share Threshold':
        ...
        df_complex = pd.DataFrame({
            ...
            '30% Profit Share Above 6% (₹)': [int(share) for share in profit_shares],
        ...
        fig_complex.add_trace(go.Bar(
            x=[f'Year {df_complex["Year"][i]}' for i in range(len(df_complex))],
            y=[df_complex["30% Profit Share Above 6% (₹)"][i] for i in range(len(df_complex))],
            name='30% Profit Share Above 6%',
            text=[f'₹{df_complex["30% Profit Share Above 6% (₹)"][i]:,}' for i in range(len(df_complex))],
            textposition='inside',
    ...
    elif model_choice == 'Simple Model with Conditional 40% Profit Share':
        st.markdown("### Simple Model with Conditional 40% Profit Share")
        ...
        df_simple = pd.DataFrame({
            ...
            '40% Profit Share Above Threshold (₹)': profit_shared,  # Profit shared
        ...
        fig_simple.add_trace(go.Bar(
            x=[f'Year {df_simple["Year"][i]}' for i in range(len(df_simple))],
            y=[df_simple["40% Profit Share Above Threshold (₹)"][i] for i in range(len(df_simple))],
            name='40% Profit Share Above Threshold',
            text=[f'₹{df_simple["40% Profit Share Above Threshold (₹)"][i]:,}' for i in range(len(df_simple))],
            textposition='inside',
            marker=dict(color='#1E90FF'),
            showlegend=True
        ))
    ...
    <div style="background-color: #F0F4F8; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 15px rgba(0, 0, 0, 0.1);">
        ...
        <ul style="list-style-type: none; padding-left: 0; font-size: 16px; color: #1E2D39;">
            ...
            <strong>Fees and Charges:</strong> For <strong>Model 1</strong>, the management fees are 5% annually for capital amounts up to ₹300,000, and 4% annually for amounts exceeding ₹300,000, with an additional 30% performance fee applied to returns exceeding 6% per year.\nIn <strong>Model 2</strong>, a 40% performance fee is applied to returns above a specified threshold.</li>
...
if page == "Resources & Contact":
    ...
    if submit_button:
        ...
        if not name or not phone or not email or not date:
            ...
            csv_file_path = 'Clients_PMS_Data(40% sharing model).csv'
